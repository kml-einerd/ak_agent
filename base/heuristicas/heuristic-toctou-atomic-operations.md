...
**RATIONALE:**
1. `increment!` does three things: reads the current value, adds 1 in the application language, writes back — two simultaneous requests both read 4, both write 5, and the user gets two downloads when they should get one (classic TOCTOU bug). [explicit]
2. This bug never appears in development with a single browser but always appears in production with concurrent requests, making it invisible during testing. [derived]
3. An atomic SQL `UPDATE WHERE` performs the check and update in a single database operation — the database guarantees only one request wins, and the `== 1` return value confirms success/failure without a separate query. [explicit]
