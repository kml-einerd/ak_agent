...
**RATIONALE:** In a chain like PublishAndSendJob → PublishToBlogJob → SendNewsletterJob, if the first job fails catastrophically (process killed, server crash, unhandled exception), the entire chain stops. A cron job scheduled slightly later acts as a safety net: it checks the expected outcome (was the newsletter sent?) and triggers the relevant job if not. Because the job is idempotent (running it when work is already done is a no-op), the cron is always safe to execute — it either does the work or returns immediately.
