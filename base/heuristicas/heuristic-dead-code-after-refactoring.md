...
**RATIONALE:** LLMs operate within a context window that cannot encompass all project files simultaneously. When moving code from file A to file B, they reliably update file B but frequently forget to remove the original code from file A. This dead code causes no immediate bugs (it's simply unused), making it invisible during testing. Over multiple refactorings, dead code accumulates and creates confusion about which version of a function is actually active.
