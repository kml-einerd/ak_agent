...
**RATIONALE:** LLMs fail in "creative" ways that don't raise exceptions: truncated responses that parse as valid JSON with fewer items, missing sections that leave no trace, format changes that produce valid but wrong structures. A generation job can return success (exit code 0) while producing a Markdown file with 2 items instead of 10, or missing required markers like [COMMENTARY] that downstream assembly depends on. Preflight checks — file exists? item count >= minimum? required markers present? data-specific constraints met? — catch these silent failures before they propagate. The check should produce a per-section status (pass, degraded, fail, skip) that gates downstream processing.
