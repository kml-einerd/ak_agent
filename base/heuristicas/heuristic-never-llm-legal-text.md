...
**RATIONALE:** LLMs produce summarized, incomplete, or subtly incorrect versions of legal text. A license with wrong wording or missing clauses may not provide the legal protection intended. Legal documents require exact language — there is no "compiler" for laws that catches errors. An AGPL-3.0 license with a paraphrased clause may not hold up legally the same as the official FSF text. The LLM treats legal prose as natural language to be generated, but legal prose is a formal specification where every word is deliberate. Unlike code (where "close enough" often works), legal text must be exactly right.
