...
**RATIONALE:**
1. Unlike human developers who skim documentation and rely on tribal knowledge, AI agents read the entire CLAUDE.MD in ~2 seconds before each interaction — every hurdle documented once is never re-discovered. [explicit]
2. Without documentation, the agent starts from zero context each session, re-encountering the same problems ("Yahoo Finance does TLS fingerprinting", "HN RSS changes format", "LLM softens opinions unless explicit"). [explicit]
3. Akita's CLAUDE.MD grew to 702 lines documenting 12 common hurdles and 14 design patterns over 8 days — institutional knowledge that would take a human weeks to absorb was available to the agent instantly in every session. [explicit]
